...
代码实现了 RAG 系统中的 Prompt Engineering 模块。
它接收向量检索返回的文档片段（context chunks），
构造成结构化 Prompt，并通过 System Prompt 约束模型只能基于提供的上下文回答问题。
随后将 Context 和 User Question 拼接为 User Prompt，发送给 GPT 模型生成答案。
其核心目的是降低幻觉（Hallucination），提升知识库问答的准确性，
是 Retrieval 和 Generation 之间的桥梁。
